...
<div id="dialogbox">
  <div>
	...
	<div id="body" class="movies">
	 ...
	 $fetch=mysql_query("SELECT * FROM `accounts` WHERE email='$email'and lname='$lastname'");
     ...
     echo "
		<script>
			setInterval(
				function(){	window.location='recovery.php?email=$email' },100
			);
		</script>";
	 }
	 else
	 echo "<script>Alert.render('The given email address and last name is not yet registered.Please register to access user`s page.')</script>";
      }
  ...
  <p style="margin-left:10px; margin-top:10px; font-family: Berlin Sans FB Demi; text-shadow: rgb(255, 0, 0) 6px 5px 5px; font-size: 20px; color: white; font-weight:bolder">Last Name:</p>
  <p style="margin-left:150px;  margin-top:-20px;"> <input name="lastname" type="text" size="30" required="required"/><br>
  <br /><br />
