...
<table class="table table-bordered" style="width:550px">
	<tr>
		<th>ID</th><th>Title</th><th>Time</th><th>Date</th><th>Seat</th><th>Status</th>
	</tr>
	<?php
		$result=$dbh->prepare("Select * From seat  where status='unavailable'");
		$result->execute();
		...
		//$ticketnum=$row['ticket_num']
			...
			{
				echo "<td><a href='delete_seat.php?userid=$row[Id]' class='btn btn-success'>".$row['status']."</a></td>";
			}else
			{
				echo "<td><a href='delete_seat.php?userid=$row[Id]' class='btn btn-warning'>".$row['status']."</a></td>";
			}
