...
#selected-seats li{float:left; width:72px; height:26px; line-height:26px; border:1px solid #d3d3d3; background:#f7f7f7; margin:6px; font-size:14px; font-weight:bold; text-align:center}
   ...
   $ticketnum=createRandomPassword();   
         ...
         ?>
			...
			<p>Your amount is: <span><?php echo $credit ?></span></p>
			<?php
			}
			?>
			<p>Seat: </p>
		          ...
		          $('.checkout-button').click(function () {
                   ...
                   if(credit>=Total)
				   {
					   $.ajax({
								...
								data: {'Arrayval':str,'ticketval':ticket,'Totalval':Total,'titleval':title,'timeval':time,'dateval':date,'ID':ID,'ticketnum':ticketnum,'username':username},
								success: function(data) {
								//param1: $(this).attr(sc.find('selected').length+1);
								//param2: $(this).attr(recalculateTotal(sc)+price); 
								
								 alert('Thanks for reservation');
							
				window.location='userrecite.php' 
				 ...
				 {
                  alert('Please recharge your credit');				 
